...
\begin{center}
{\bf Intro to Data Science \\ Homework 5: Due Wednesday October 23 at 2:00pm}
\end{center}
...
Many of the exercises for this assignment ask you to use the resampling methods (bootstrap and permutation tests) introduced in class in the R notebook basicStatsNotebook.   
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item The following command will produce all possible permutations (note that there are $5!=$\Sexpr{factorial(5)}) of the values 8, 10, 15, 5, and 9:
      <<results="hide">>=
