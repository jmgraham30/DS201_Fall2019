...
The book [R for Data Science](https://r4ds.had.co.nz/tidy-data.html) provides a practical introduction to tidy data with R in mind while the book [Pandas for Everyone](https://github.com/chendaniely/pandas_for_everyone) provides a practical introduction to tody data with Python (using Pandas) in mind. 
...
The data is **not** tidy. The reason is because each observation corresponds to one student and the relevant variables are Student, Subject, and Grade. In this cas, the Grade variable is spread out across multiple columns. Lucky for us, R contains (in the tidyr package) a number of functions that make it easy to "tidy" this data. For example,
```{r}
