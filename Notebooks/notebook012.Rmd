...
```{r}
temp_res <- predict(default_glm_mod$finalModel,Default_test,type = "response") %>%
 prediction(Default_test$default) %>% performance("auc")
```
...
```{r}
ggplot(data=NULL,mapping=aes(x=temp_rates@x.values[[1]],y=temp_rates@y.values[[1]])) + geom_line()
```
