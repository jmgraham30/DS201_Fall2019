...
```{r}
train_result <- train(Species~.,data=iris_train,trControl=tC,tuneGrid=tG,method="knn",preProcess = c("center", "scale"))
```
...
```{r}
best_preds <- predict(train_result,newdata = iris_test[ ,1:4])

confusionMatrix(best_preds,iris_test$Species)
```


```{r}
iris_test$pred_vals <- best_preds

iris_test %>% ggplot(aes(x=Petal.Length,y=Petal.Width,color=Species,shape=pred_vals)) + 
  geom_point()
```
