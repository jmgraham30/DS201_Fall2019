...
```{r}
(return_rss(c(2.812191,1.848551)))
```
...
```{r}
(test_data_frame <- data.frame(one=c(6,5,4,3,2),two=c(-1,4,-2,3,0)))
```


```{r}
(sampled_ids <- sample(1:5,replace = T))
```


```{r}
test_data_frame[sampled_ids, ]
```
...
2) Cross-validation to estimate test error
3) The topic of nonlinearity
4) Overfitting 
5) Model and feature selection 

These topics will be addressed in the next few notebooks. 
